...
with TaskManager(cpus_per_task=2, use_all_cpus=True) as tm:
	...
	for input_file in tm.iterate(input_files):
		...
		output_name_pk = 'EZmock_Nmesh512_pk2D_kmu/%i.txt' % int(rank)
		output_name_pk_l = 'EZmock_Nmesh512_pkl/%i.txt' % int(rank)
		output_name_pk_kpar_kperp = 'EZmock_Nmesh512_pk2D_kpar_kperp/%i.txt' % int(rank)
		...
		zrsd = cat['z'][:,None]  + cat['vz'][:,None]  * (1 + redshift)/H
		x = cat['x'][:,None]
		y = cat['y'][:,None]
		...
		cond = ~np.isnan(zrsd)
		zrsd = zrsd[cond]
		zrsd[zrsd > BoxSize] = zrsd[zrsd > BoxSize] - BoxSize
		zrsd[zrsd < 0] = zrsd[zrsd < 0] + BoxSize
